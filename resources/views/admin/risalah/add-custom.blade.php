...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // =========================
        const oldTujuan = @json(old('tujuan', []));
        ...
        function getSelectedTujuanCount() {
            return document.querySelectorAll('#tujuan-container input[name="tujuan[]"]').length;
        }

        function hideTujuanError() {
            if (tujuanError) {
                tujuanError.style.display = 'none';
                tujuanError.innerText = 'Minimal pilih satu tujuan!';
            }
        }

        function showTujuanError(message = 'Minimal pilih satu peserta acara!') {
            if (tujuanError) {
                tujuanError.style.display = 'block';
                tujuanError.innerText = message;
            }
        }
        ...
        function updateNomor() {
            ...
            items.forEach((item, index) => {
                ...
                item.querySelectorAll('.sub-risalah-row').forEach((subRow, subIndex) => {
                    ...
                    if (badge) {
                        badge.innerHTML = `<i class="fas fa-code-branch me-1"></i> Sub ${subIndex + 1}`;
                    }
        ...
        function restoreOldRisalah() {
            ...
            // Kalau belum ada old, tampilkan 1 item kosong
        ...
        if (risalahForm && submitBtn) {
            risalahForm.addEventListener('submit', function(e) {
                ...
                if (getSelectedTujuanCount() < 1) {
                    e.preventDefault();
                    showTujuanError('Minimal pilih satu peserta acara!');
                    const orgTree = document.getElementById('org-tree');
                    if (orgTree) {
                        orgTree.scrollIntoView({
                            behavior: 'smooth',
                            block: 'center'
                        });
                    }
        ...
        function refreshSelectedRecipients(selectedNodes, userIds) {
            if (!selectedRecipients || !selectedSection || !tujuanContainer) return;

            tujuanContainer.innerHTML = '';

            userIds.forEach(function(nodeId) {
                tujuanContainer.insertAdjacentHTML(
                    'beforeend',
            ...
            selectedNodes.sort(function(a, b) {
                ...
                function getPriority(text) {
                    for (let pos in positionOrder) {
                        if (String(text).startsWith(pos)) return positionOrder[pos];
                    }
                    return 999;
                }
            ...
            if (selectedNodes.length) {
                selectedNodes.forEach(function(name) {
                    selectedRecipients.insertAdjacentHTML('beforeend', '<li>' + escapeHtml(name) + '</li>');
                });
        ...
        try {
            $('#org-tree')
                .on('ready.jstree', function() {
                    if (Array.isArray(oldTujuan) && oldTujuan.length > 0) {
                        oldTujuan.forEach(function(id) {
                            $('#org-tree').jstree(true).select_node(String(id));
                        });
                    }
                })
                .jstree({
                ...
                .on('changed.jstree', function(e, data) {
                    const allSelectedNodes = data.instance.get_selected(true);
                    const selectedNodes = [];
                    const userIds = [];
                    ...
                    refreshSelectedRecipients(selectedNodes, userIds);
                });
        } catch (error) {
