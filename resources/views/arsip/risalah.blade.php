...
@section('content')
    <div class="container-fluid px-4 py-0 mt-0">
        <div class="card shadow-sm border-0">
            <div class="card-body py-3">
                ...
                <h3 class="fw-bold mb-3">Arsip Risalah</h3>
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <div class="bg-white border rounded-2 px-3 py-2 w-100 d-flex align-items-center">
                            ...
                            <span class="text-muted ms-1">/ Arsip Risalah</span>
                        </div>
                    </div>
                </div>

                {{-- Row Filter --}}
                <form class="row g-2 align-items-center" method="GET" action="{{ route('arsip.risalah') }}">
                    <div class="col-auto">
                    ...
                    <table class="table table-bordered custom-table-bagian">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" style="width:14%;">Tanggal Masuk</th>
                        ...
                        <tbody>
                            @if (isset($arsipRisalah) && $arsipRisalah->count() > 0)
                                @foreach ($arsipRisalah as $index => $arsip)
                                    @php
                                        $risalah = $arsip->document;
                                    @endphp
                                    @if ($risalah)
                                        <tr>
                                            <td class="nomor">{{ ($arsipRisalah->firstItem() ?? 0) + $index }}</td>
                                        ...
                                        {{ $risalah->status == 'reject' ? 'text-danger' : ($risalah->status == 'correction' ? 'text-warning' : ($risalah->status == 'approve' ? 'text-success' : '')) }}"
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($risalah->tgl_dibuat)->format('d-m-Y') ?? '-' }}
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    <button title="Detail"
                                                        ...
                                                        onclick="window.location.href='{{ route('view.risalah-arsip', $arsip->document->id_risalah) }}'">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        onclick="showRestoreConfirmation({{ $risalah->id_risalah }}, '{{ $risalah->judul ?? $risalah->nama_dokumen }}')"
                                                        title="Pulihkan">
                                                    ...
                                                    <button title="Unduh"
                                                        ...
                                                        style="width:30px; height:30px; display:flex; align-items:center; justify-content:center;"
                                                        onclick="window.location.href='{{ route('cetakrisalah', ['id' => $arsip->document->id_risalah]) }}'">
                            ...
                            @else
                                <tr>
                                    <td colspan="10" class="text-center">Tidak ada data arsip risalah</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
                ...
                <div class="d-flex justify-content-end mt-3">
                    {{ $arsipRisalah->onEachSide(1)->appends(request()->query())->links('pagination::bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="deleteRisalahModal" tabindex="-1" aria-labelledby="deleteRisalahModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteRisalahModalLabel">Hapus Permanen Risalah</h5>
    ...
    <script>
        ...
        // ---------- Restore dengan SweetAlert ----------
        function showRestoreConfirmation(risalahId, risalahTitle) {
            currentRisalahId = risalahId;
            Swal.fire({
                title: 'Pulihkan Risalah?',
                html: `Apakah Anda yakin ingin memulihkan risalah <strong>${risalahTitle}</strong>?<br><br>
                       <span class="text-muted">Risalah yang dipulihkan akan dikembalikan ke daftar utama.</span>`,
                icon: 'question',
        ...
        function confirmRestore() {
            ...
            fetch(`/arsip/${currentRisalahId}/Risalah`, {
                    method: 'DELETE',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            title: 'Berhasil!',
                            text: 'Risalah berhasil dipulihkan',
                            icon: 'success',
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @if (session('success') === 'Risalah terpilih berhasil dihapus permanen.')
                showNotification('Risalah terpilih berhasil dihapus permanen.', 'success');
            @endif

            @if (session('success') === 'Risalah terpilih berhasil dipulihkan.')
                showNotification('Risalah terpilih berhasil dipulihkan.', 'success');
            @endif
        });
    </script>
@endsection
