                                        ...
                                        $(function() {
                                            ...
                                            function initRecipientTree(treeSelector, preSelected, selectedListSelector, sectionSelector,
                                                ...
                                                $(treeSelector).jstree({
                                                    ...
                                                    function isOrgNode(nodeId) {
                                                        return nodeId.startsWith('divisi-') ||
                                                            nodeId.startsWith('dept-') ||
                                                            nodeId.startsWith('section-') ||
                                                            nodeId.startsWith('unit-') ||
                                                            nodeId.startsWith('director-');
                                                    }

                                                    function isUserNode(nodeId) {
                                                        return nodeId.startsWith('user-');
                                                    }

                                                    function hasSelectedOrgAncestor(node, instance) {
                                                        var parentId = node.parent;

                                                        while (parentId && parentId !== '#') {
                                                            var parentNode = instance.get_node(parentId);
                                                            if (parentNode && parentNode.state && parentNode.state.selected && isOrgNode(
                                                                    parentNode.id)) {
                                                                return true;
                                                            }
                                                            parentId = parentNode ? parentNode.parent : '#';
                                                        }
                                                    ...
                                                    // =========================
                                                    // LIST TAMPILAN YANG DIRINGKAS
                                                    // =========================
                                                    var displayNodes = selectedNodes.filter(function(node) {
                                                        // kalau node punya parent organisasi yang juga terpilih, jangan tampilkan
                                                        return !hasSelectedOrgAncestor(node, data.instance);
                                                    });

                                                    // kalau mau, kita bisa urutkan agar org tampil dulu, user belakangan
                                                    displayNodes.sort(function(a, b) {
                                                        const aOrg = isOrgNode(a.id) ? 0 : 1;
                                                        const bOrg = isOrgNode(b.id) ? 0 : 1;
                                                        return aOrg - bOrg;
                                                    });
                                                    ...
                                                    if (displayNodes.length) {
                                                        displayNodes.forEach(function(node) {
                                                            ...
                                                            if (node.id.startsWith('divisi-')) {
                                                                label = 'Divisi: ' + node.text;
                                                            } else if (node.id.startsWith('dept-')) {
                                                                label = 'Department: ' + node.text;
                                                            } else if (node.id.startsWith('section-')) {
                                                                label = 'Section: ' + node.text;
                                                            } else if (node.id.startsWith('unit-')) {
                                                                label = 'Unit: ' + node.text;
                                                            } else if (node.id.startsWith('director-')) {
                                                                label = 'Direktorat: ' + node.text;
                                                            }
                                                    ...
                                                    if (inputName === 'tujuan[]' && selectedNodes.length > 0) {
                                                        $('#errorTujuan').hide();
                                                    }
                                                });
                                            }
                            ...
                            <div class="col-md-12">
                                <label for="bcc-tree" class="form-label">
                                    ...
                                    BCC (Blind Carbon Copy) <span class="text-muted form-text"
                                        style="font-size: x-small;">(Kosongkan jika
                                        tidak ada.)</span>
                                </label>
                                <div class="border rounded p-2" style="max-height: 300px; overflow-y: auto;">
